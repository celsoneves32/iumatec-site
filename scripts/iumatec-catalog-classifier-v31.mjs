...
/*
 IUMATEC Catalog Classifier V3.1 — CONSERVATIVE / TITLE-FIRST / DRY RUN ONLY
 =========================================================================
 Objetivo:
...
const ACCESSORY = [
  ...
  /\bprivacy\b/, /\bprivacy\s+filter\b/, /\bfilter\b/, /\bblickschutzfilter\b/, /\bsichtschutzfilter\b/, /\bscreen\s+filter\b/,
...
const KEYBOARD_ANY = [/\bkeyboard\b/, /\btastatur\b/];
const MOUSE_ANY = [/\bmouse\b/, /\bmaus\b/];
...
const MONITOR_ACCESSORY = [
  /\bprivacy\b/, /\bfilter\b/, /\bblickschutzfilter\b/, /\bsichtschutzfilter\b/,
  /\bscreen\s+filter\b/, /\bhalterung\b/, /\bmount\b/,
...
const MOTHERBOARD_ACCESSORY = [
  /\bcable\b/, /\bkabel\b/, /\bextension\b/, /\bverlangerung\b/,
  /\bconnector\b/, /\b24\s*pin\b/, /\b20\+?4\s*pin\b/
];
const RAM = [
...
const PRINTER_DEVICE = [
  /\bdrucker\b/, /\blaserjet\b/, /\bofficejet\b/, /\bdeskjet\b/,
  /\becotank\b/, /\bsmart\s+tank\b/,
  /\bhp\s+envy\s+\d{4}[a-z]?\b/,
...
function classify(p) {
  ...
  // V3.1: impressoras antes de Desktop-PC.
  // "All-in-One" é muito comum em impressoras e deixou de ser assinatura de desktop.
  if (hit(t, PRINTER_CONSUMABLE)) {
    return out("printer_consumable", "Office & Business", "Drucker & Scanner", 0.999, "printer-consumable");
  }

  if (hit(t, PRINTER_DEVICE) && !hit(t, PHOTO_SCANNER_BLOCK)) {
    return out("printer_scanner", "Office & Business", "Drucker & Scanner", 0.999, "printer-device");
  }
  ...
  if (hit(t, MOTHERBOARD) && hit(t, MOTHERBOARD_ACCESSORY)) {
    return out("pc_cable_adapter", "PC-Komponenten", "Kabel & Adapter", 0.999, "mainboard-cable-accessory");
  }
  ...
  // V3.1: conjuntos Tastatur + Maus ficam em Tastaturen, nunca são empurrados para Mäuse.
  if (hit(t, KEYBOARD_ANY) && hit(t, MOUSE_ANY) && !hit(t, KEYBOARD_FALSE)) {
    return out("keyboard_mouse_combo", "Peripherie", "Tastaturen", 0.999, "keyboard-mouse-combo");
  }
...
function runRegressionTests() {
  const cases = [
    {
      name: "monitor privacy filter stays accessory",
      p: { title: '3M PF340W2E Blickschutzfilter Standard für Monitor 34" 21:9', brand: "3M", category: "Peripherie", subcategory: "Zubehör" },
      expect: ["Peripherie", "Zubehör"]
    },
    {
      name: "HP DeskJet is printer, not desktop",
      p: { title: "HP DeskJet 2910 All-in-One 3 in 1, A4, USB 2.0, WLAN", brand: "HP Inc.", category: "Peripherie", subcategory: "Zubehör" },
      expect: ["Office & Business", "Drucker & Scanner"]
    },
    {
      name: "keyboard mouse set stays keyboards",
      p: { title: "Dell KM5221 Wireless-Tastatur und Maus CH-Layout", brand: "Dell", category: "Peripherie", subcategory: "Tastaturen" },
      expect: ["Peripherie", "Tastaturen"]
    },
    {
      name: "mainboard extension stays cable",
      p: { title: "ATX Extension 24-24Pin 24 Pin Mainboard Verlängerung 30cm", brand: "SilverStone", category: "PC-Komponenten", subcategory: "Kabel & Adapter" },
      expect: ["PC-Komponenten", "Kabel & Adapter"]
    },
    {
      name: "iPad device to tablets",
      p: { title: 'Apple iPad Air 512GB Purple 11", M4, Cellular, 2026', brand: "Apple", category: "Computer", subcategory: "Laptops" },
      expect: ["Mobile", "Tablets"]
    },
    {
      name: "RX graphics card to GPU",
      p: { title: "Acer Nitro AMD RX 9070 XT 16G OC AMD Radeon RX 9070 XT, HDMI, DP", brand: "Acer", category: "Peripherie", subcategory: "Monitore" },
      expect: ["PC-Komponenten", "Grafikkarten"]
    }
  ];

  const failures = [];
  for (const c of cases) {
    const r = classify(c.p);
    if (r.category !== c.expect[0] || r.subcategory !== c.expect[1]) {
      failures.push(`${c.name}: ${r.category} > ${r.subcategory}`);
    }
  }

  if (failures.length) {
    throw new Error("REGRESSION TESTS FALHARAM:\n" + failures.join("\n"));
  }

  console.log(`REGRESSION TESTS: ${cases.length}/${cases.length} OK`);
}
...
const suspiciousPrinterIntoDesktop = safe.filter(
  (r) =>
    r.proposed_category === "Computer" &&
    r.proposed_subcategory === "Desktop-PCs" &&
    hit(n(r.title), PRINTER_DEVICE)
);

const suspiciousComboIntoMouse = safe.filter(
  (r) =>
    r.proposed_category === "Peripherie" &&
    r.proposed_subcategory === "Mäuse" &&
    hit(n(r.title), KEYBOARD_ANY) &&
    hit(n(r.title), MOUSE_ANY)
);

const suspiciousCableIntoMainboard = safe.filter(
  (r) =>
    r.proposed_category === "PC-Komponenten" &&
    r.proposed_subcategory === "Mainboards" &&
    hit(n(r.title), MOTHERBOARD_ACCESSORY)
);

const suspiciousFilterIntoMonitor = safe.filter(
  (r) =>
    r.proposed_category === "Peripherie" &&
    r.proposed_subcategory === "Monitore" &&
    hit(n(r.title), MONITOR_ACCESSORY)
);

const guardTotal =
  suspiciousAccessoryIntoDevice.length +
  suspiciousRobotIntoMobile.length +
  suspiciousLabelKeyboard.length +
  suspiciousPrinterIntoDesktop.length +
  suspiciousComboIntoMouse.length +
  suspiciousCableIntoMainboard.length +
  suspiciousFilterIntoMonitor.length;

const stamp = new Date().toISOString().replaceAll(":", "-").replaceAll(".", "-");
const allPath = path.join(OUT, `catalog-classifier-v31-all-${stamp}.csv`);
const safePath = path.join(OUT, `catalog-classifier-v31-safe-changes-${stamp}.csv`);
const riskPath = path.join(OUT, `catalog-classifier-v31-page-risk-${stamp}.csv`);
const guardPath = path.join(OUT, `catalog-classifier-v31-guard-failures-${stamp}.csv`);
const summaryPath = path.join(OUT, `catalog-classifier-v31-summary-${stamp}.txt`);

writeCsv(allPath, rows);
writeCsv(safePath, safe);
writeCsv(riskPath, [...riskYes, ...riskUnknown]);
writeCsv(guardPath, [
  ...suspiciousAccessoryIntoDevice,
  ...suspiciousRobotIntoMobile,
  ...suspiciousLabelKeyboard,
  ...suspiciousPrinterIntoDesktop,
  ...suspiciousComboIntoMouse,
  ...suspiciousCableIntoMainboard,
  ...suspiciousFilterIntoMonitor
]);
...
const L = [];
L.push("========== IUMATEC CATALOG CLASSIFIER V3.1 ==========");
...
L.push(`Impressoras propostas como Desktop-PCs: ${suspiciousPrinterIntoDesktop.length}`);
L.push(`Tastatur+Maus propostos como Mäuse: ${suspiciousComboIntoMouse.length}`);
L.push(`Cabos/Extensões propostos como Mainboards: ${suspiciousCableIntoMainboard.length}`);
L.push(`Filtros/Privacy propostos como Monitore: ${suspiciousFilterIntoMonitor.length}`);
...
console.log("DRY RUN V3.1 concluído. NENHUM produto foi alterado.");
