...
/*
 IUMATEC PAGE CLEANUP V1.1 — DRY RUN ONLY
 ======================================
...
function smartphoneRule(p) {
  ...
  // Séries conhecidas que por vezes não dizem explicitamente "case".
  if (
    ...
    (brand === "pioneer" && /\bsmartphone\b/.test(t))
  ) {
    return result(
      "SAFE",
      "Mobile",
  ...
  // Leather/leder + iPhone numa marca de acessórios é capa/etui.
  if (
...
const safePath = path.join(OUT, `page-cleanup-v11-safe-${stamp}.csv`);
const reviewPath = path.join(OUT, `page-cleanup-v11-review-${stamp}.csv`);

writeCsv(safePath, proposals);
writeCsv(reviewPath, reviews);

console.log("");
console.log("========== IUMATEC PAGE CLEANUP V1.1 ==========");
