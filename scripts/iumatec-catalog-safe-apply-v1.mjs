...
/*
 IUMATEC Catalog Safe Apply Gate V1
 ==================================
...
if (APPLY && CONFIRM !== "IUMATEC-CATALOG-SAFE-V1") {
  throw new Error(
    "APPLY bloqueado. Use: --apply --confirm=IUMATEC-CATALOG-SAFE-V1"
  );
}
...
function decision(row) {
  ...
  if (
    ...
    return ["BLOCK", "accessory-into-monitor"];
  }
  ...
  if (
    ...
    /case|cover|folio|filter|privacy|halter|holder|mount|stand|bag|tasche|sleeve|cable|kabel|adapter|dock|charger|ladegerat|powerbank|glass|schutz|cage/.test(
      title
    )
  ) {
    return ["BLOCK", "accessory-into-device-page"];
  }
  ...
  if (
    ...
    return ["BLOCK", "printer-into-desktop"];
  }
  ...
  if (
    ...
    return ["BLOCK", "cable-into-mainboard"];
  }
  ...
  if (
    ...
    return ["BLOCK", "robot-into-mobile"];
  }
...
const candidatesPath = path.join(
  OUT,
  `catalog-safe-apply-v1-candidates-${stamp}.csv`
);
const heldPath = path.join(
  OUT,
  `catalog-safe-apply-v1-held-${stamp}.csv`
);
const blockedPath = path.join(
  OUT,
  `catalog-safe-apply-v1-blocked-${stamp}.csv`
);
const applyPath = path.join(
  OUT,
  `catalog-safe-apply-v1-${mode}-${stamp}.csv`
);
...
console.log("");
console.log("========== IUMATEC CATALOG SAFE APPLY GATE V1 ==========");
...
if (!APPLY) {
  ...
  console.log("");
  console.log("Só se BLOCKED = 0 e a amostra estiver correta:");
  console.log(
    "npx @dotenvx/dotenvx run -f .env.local -- node " +
    ".\\scripts\\iumatec-catalog-safe-apply-v1.mjs " +
    "--apply --confirm=IUMATEC-CATALOG-SAFE-V1"
  );
} else {
