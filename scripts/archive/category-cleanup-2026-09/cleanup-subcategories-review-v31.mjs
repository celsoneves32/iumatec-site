...
if (APPLY && CONFIRM !== "IUMATEC-REVIEW-V31") {
  throw new Error(
    "APPLY bloqueado. Use: --apply --confirm=IUMATEC-REVIEW-V31"
  );
}
...
function classify(row) {
  ...
  if (oldCat === "Computer" && oldSub === "Laptops") {
    ...
    if (any(t, TABLET_ACCESSORY)) {
      return move(row, "Mobile", "Mobile Zubehör", "v31:tablet-accessory");
    }

    if (any(t, DOCK)) {
      return move(row, "Peripherie", "Dockingstationen", "v31:dockingstation");
    }

    if (any(t, KEYBOARD)) {
      return move(row, "Peripherie", "Tastaturen", "v31:keyboard");
    }

    if (any(t, COMPUTER_ACCESSORY)) {
      return move(row, "Computer", "Computer-Zubehör", "v31:computer-accessory");
    }

    // 2) Actual device classes.
    if (any(t, TABLET_DEVICE)) {
      return move(row, "Mobile", "Tablets", "v31:tablet-device");
    }

    if (any(t, MINI_PC)) {
      return move(row, "Computer", "Mini-PCs", "v31:mini-pc");
    }

    if (any(t, DESKTOP_PC)) {
      return move(row, "Computer", "Desktop-PCs", "v31:desktop-pc");
    }

    if (any(t, LAPTOP_DEVICE)) {
      return keep(row, "v31:true-laptop");
    }
  }

  if (oldCat === "Mobile") {
    if (any(t, PHONE_ACCESSORY) || any(t, TABLET_ACCESSORY)) {
      return move(row, "Mobile", "Mobile Zubehör", "v31:mobile-accessory");
    }

    if (any(t, SMART_HOME_BUILDING)) {
      return move(row, "Smart Home", "Gebäudetechnik", "v31:smart-home-building");
    }

    if (any(t, SMART_HOME_ENERGY)) {
      return move(row, "Smart Home", "Energie & Strom", "v31:smart-home-energy");
    }
    ...
    if (any(t, SMART_HOME_SECURITY)) {
      return move(
        ...
        "v31:smart-home-security"
      );
    }

    if (any(t, FOTO_VIDEO)) {
      return move(row, "Peripherie", "Foto & Video", "v31:foto-video");
    }

    if (any(t, TABLET_DEVICE)) {
      if (oldSub === "Tablets") return keep(row, "v31:true-tablet");
      return move(row, "Mobile", "Tablets", "v31:tablet-device");
    }

    if (any(t, PHONE_DEVICE)) {
      if (oldSub === "Smartphones") return keep(row, "v31:true-phone");
      return move(row, "Mobile", "Smartphones", "v31:phone-device");
    }

    if (any(t, LAPTOP_DEVICE)) {
      return move(row, "Computer", "Laptops", "v31:laptop-device");
    }
  }
...
const report = path.join(
  OUT,
  `subcategory-review-v31-${mode}-${stamp}.csv`
);

const unresolvedReport = path.join(
  OUT,
  `subcategory-review-v31-unresolved-${stamp}.csv`
);
...
console.log("");
console.log("TOP TRANSICOES V3.1:");
...
if (!APPLY) {
  console.log("");
  console.log("DRY RUN V3.1 concluido. NENHUM produto foi alterado.");
  console.log("IMPORTANTE: reveja a amostra antes de APPLY.");
  console.log("");
  console.log("APPLY apenas depois de aprovado:");
  console.log(
    "node .\\scripts\\cleanup-subcategories-review-v31.mjs " +
    "--apply --confirm=IUMATEC-REVIEW-V31"
  );
} else {
  console.log("");
  console.log("APPLY V3.1 concluido.");
