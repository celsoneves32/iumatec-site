...
if (APPLY && CONFIRM !== "IUMATEC-REVIEW-V33") {
  throw new Error(
    "APPLY bloqueado. Use: --apply --confirm=IUMATEC-REVIEW-V33"
  );
}
...
function hasLaptopPrivacyContext(t) {
  return any(t, [
    "surface book",
    "dragonfly",
    "elitebook",
...
function classify(row) {
  ...
  if (oldCat === "Computer" && oldSub === "Laptops") {
    ...
    if (isPrivacyFilter(t)) {
      if (hasLaptopPrivacyContext(t)) {
        return move(
          row,
          "Computer",
          "Computer-Zubehör",
          "v33:laptop-privacy-accessory"
        );
      }
    ...
    if (any(t, TABLET_ACCESSORY)) {
      return move(row, "Mobile", "Mobile Zubehör", "v33:tablet-accessory");
    }

    if (any(t, DOCK)) {
      return move(row, "Peripherie", "Dockingstationen", "v33:dockingstation");
    }

    if (any(t, KEYBOARD)) {
      return move(row, "Peripherie", "Tastaturen", "v33:keyboard");
    }

    if (any(t, COMPUTER_ACCESSORY)) {
      return move(row, "Computer", "Computer-Zubehör", "v33:computer-accessory");
    }

    // 2) Actual device classes.
    if (any(t, TABLET_DEVICE)) {
      return move(row, "Mobile", "Tablets", "v33:tablet-device");
    }

    if (any(t, MINI_PC)) {
      return move(row, "Computer", "Mini-PCs", "v33:mini-pc");
    }

    if (any(t, DESKTOP_PC)) {
      return move(row, "Computer", "Desktop-PCs", "v33:desktop-pc");
    }

    if (any(t, LAPTOP_DEVICE)) {
      return keep(row, "v33:true-laptop");
    }
  }

  if (oldCat === "Mobile") {
    if (any(t, PHONE_ACCESSORY) || any(t, TABLET_ACCESSORY)) {
      return move(row, "Mobile", "Mobile Zubehör", "v33:mobile-accessory");
    }

    if (any(t, SMART_HOME_BUILDING)) {
      return move(row, "Smart Home", "Gebäudetechnik", "v33:smart-home-building");
    }

    if (any(t, SMART_HOME_ENERGY)) {
      return move(row, "Smart Home", "Energie & Strom", "v33:smart-home-energy");
    }
    ...
    if (any(t, SMART_HOME_SECURITY)) {
      return move(
        ...
        "v33:smart-home-security"
      );
    }

    if (any(t, FOTO_VIDEO)) {
      return move(row, "Peripherie", "Foto & Video", "v33:foto-video");
    }

    if (any(t, TABLET_DEVICE)) {
      if (oldSub === "Tablets") return keep(row, "v33:true-tablet");
      return move(row, "Mobile", "Tablets", "v33:tablet-device");
    }

    if (any(t, PHONE_DEVICE)) {
      if (oldSub === "Smartphones") return keep(row, "v33:true-phone");
      return move(row, "Mobile", "Smartphones", "v33:phone-device");
    }

    if (any(t, LAPTOP_DEVICE)) {
      return move(row, "Computer", "Laptops", "v33:laptop-device");
    }
  }
...
const report = path.join(
  OUT,
  `subcategory-review-v33-${mode}-${stamp}.csv`
);

const unresolvedReport = path.join(
  OUT,
  `subcategory-review-v33-unresolved-${stamp}.csv`
);
...
console.log("");
console.log("TOP TRANSICOES V3.3:");
...
if (!APPLY) {
  console.log("");
  console.log("DRY RUN V3.3 concluido. NENHUM produto foi alterado.");
  console.log("IMPORTANTE: reveja a amostra antes de APPLY.");
  console.log("");
  console.log("APPLY apenas depois de aprovado:");
  console.log(
    "node .\\scripts\\cleanup-subcategories-review-v33.mjs " +
    "--apply --confirm=IUMATEC-REVIEW-V33"
  );
} else {
  console.log("");
  console.log("APPLY V3.3 concluido.");
