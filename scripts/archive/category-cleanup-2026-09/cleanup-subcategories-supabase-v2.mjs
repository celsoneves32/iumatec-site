...
if (APPLY && CONFIRM !== "IUMATEC-SANITY-V2") {
  throw new Error(
    "APPLY bloqueado. Use: --apply --confirm=IUMATEC-SANITY-V2"
  );
}
...
const report = path.join(
  OUT,
  `subcategory-sanity-v2-${APPLY ? "apply" : "dry-run"}-${stamp}.csv`
);
...
console.log("");
console.log("========== IUMATEC SUBCATEGORY SANITY V2 ==========");
...
if (!APPLY) {
  ...
  console.log(
    "node .\\scripts\\cleanup-subcategories-supabase-v2.mjs " +
    "--apply --confirm=IUMATEC-SANITY-V2"
  );
} else {
  console.log("");
