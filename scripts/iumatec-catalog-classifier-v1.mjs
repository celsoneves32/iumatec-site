...
/* IUMATEC Catalog Classifier V1 — DRY RUN ONLY */
...
const ACCESSORY = [
  ...
  /\bhalter\b/,/\bholder\b/,/\bmount\b/,/\bwandhalter\b/,/\bstand\b/,/\bstativ\b/,/\bstander\b/,/\bclamp\b/,/\bklemme\b/,/\bcage\b/,
  /\bdock\b/,/\bdocking\b/,/\bhub\b/,/\badapter\b/,/\bkabel\b/,/\bcable\b/,/\bcharger\b/,/\bladegerat\b/,/\bnetzteil\b/,/\bpowerbank\b/,/\bakku\b/,/\bbattery\b/,
  /\bkeyboard\b/,/\btastatur\b/,/\bmouse\b/,/\bmaus\b/,/\bstylus\b/,/\bstift\b/,/\bpencil\b/,/\bheadset\b/,/\bheadphone\b/,/\bkopfhorer\b/,/\bwebcam\b/,/\bcooler\b/,/\bkuhler\b/,/\blufter\b/,
  /\breinigung\b/,/\bcleaning\b/,/\bersatz\b/,/\breplacement\b/,/\bstrap\b/,/\bband\b/,/\bkette\b/
];
...
const PHONE=[/\bsmartphone\b/,/\bmobiltelefon\b/,/\biphone\s*(?:se|1[1-9]|[6-9])\b/,/\bgalaxy\s+(?:s|z|a|m)\s*\d{1,3}\b/,/\bgalaxy\s+xcover\b/,/\bgoogle\s+pixel\s+\d/,/\bpixel\s+\d/,/\bfairphone\s*\d*/,/\bnothing\s+phone\b/,/\boneplus\s+\d/,/\bxiaomi\s+(?:1[0-9]|mi\s*\d)\b/,/\bredmi\s+(?:note\s*)?\d/,/\bpoco\s+[cfmx]\d/i,/\bmotorola\s+(?:edge|razr)\b/,/\bmoto\s+[gex]\d/i,/\bhonor\s+(?:magic|\d)/,/\bnokia\s+[a-z]?\d/i];
...
const DESKTOP=[/\bdesktop[- ]?pc\b/,/\bdesktop\b/,/\bworkstation\b/,/\bgaming\s+pc\b/,/\ball[- ]in[- ]one\b/,/\boptiplex\b/,/\bprodesk\b/,/\belitedesk\b/,/\bprecision\s+\d/,/\btower\s+pc\b/];
const MINI=[/\bmini[- ]?pc\b/,/\bnuc\b/,/\bmini[- ]?system\b/,/\bmini\s+computer\b/,/\btiny\s+pc\b/];
const MONITOR=[/\bmonitor\b/,/\bdisplay\b/];
...
const GPU=[/\bgrafikkarte\b/,/\bgraphics\s+card\b/,/\bgeforce\s+(?:rtx|gtx)\b/,/\brtx\s*\d{3,4}\b/,/\bradeon\s+rx\s*\d/];
const RAM=[/\b(?:ddr4|ddr5)\b.*\b(?:8|16|32|64|128)\s*gb\b/,/\barbeitsspeicher\b/,/\bmemory\s+kit\b/];
...
const STORAGE=[/\bnvme\b/,/\bssd\b/,/\bhdd\b/,/\bhard\s+drive\b/,/\bfestplatte\b/,/\bnas\b/,/\bexternal\s+ssd\b/,/\bexterne\s+ssd\b/,/\bportable\s+ssd\b/];
const PRINTER=[/\bdrucker\b/,/\bprinter\b/,/\bscanner\b/,/\bmultifunktions\b/,/\blaserjet\b/,/\bofficejet\b/,/\becotank\b/,/\bworkforce\b/];
const PRINTCON=[/\btoner\b/,/\btinte\b/,/\bink\b/,/\bcartridge\b/,/\bpatrone\b/,/\btrommel\b/,/\bdrum\b/,/\bpapier\b/,/\blabel\b/,/\betikett\b/];
...
const ENERGY=[/\bsmart\s+plug\b/,/\bsteckdose\b/,/\bstromverteiler\b/,/\bwallbox\b/,/\bladestation\b/];
...
function classify(p){
  ...
  if(any(t,PHONE)&&!any(t,ACCESSORY)) return result("smartphone_device","Mobile","Smartphones",0.995,"strong smartphone model");
  if(any(t,TABLET)&&!any(t,ACCESSORY)) return result("tablet_device","Mobile","Tablets",0.995,"strong tablet model");
  if(any(t,MINI)&&!any(t,ACCESSORY)) return result("mini_pc_device","Computer","Mini-PCs",0.995,"mini pc");
  if(any(t,LAPTOP)&&!any(t,ACCESSORY)) return result("laptop_device","Computer","Laptops",0.995,"laptop family");
  if(any(t,DESKTOP)&&!any(t,ACCESSORY)) return result("desktop_pc","Computer","Desktop-PCs",0.992,"desktop family");
  if(any(t,GPU)) return result("graphics_card","PC-Komponenten","Grafikkarten",0.997,"gpu");
  if(any(t,RAM)) return result("ram_module","PC-Komponenten","RAM",0.99,"ram");
  if(any(t,MAINBOARD)) return result("motherboard","PC-Komponenten","Mainboards",0.995,"mainboard");
  if(any(t,CPU)) return result("processor","PC-Komponenten","Prozessoren",0.99,"cpu");
  if(any(t,MONITOR)&&!any(t,MONITOR_ACC)&&!any(t,ACCESSORY)) return result("monitor_device","Peripherie","Monitore",0.99,"monitor");
  if(any(t,KEYBOARD)&&!any(t,[...PHONE_CTX,...TABLET_CTX])) return result("keyboard","Peripherie","Tastaturen",0.99,"keyboard");
  if(any(t,MOUSE)) return result("mouse","Peripherie","Mäuse",0.99,"mouse");
  if(any(t,HEADSET)) return result("headset","Peripherie","Headsets",0.99,"headset");
  if(any(t,WEBCAM)&&!/abdeckung|cover|privacy/.test(t)) return result("webcam","Peripherie","Webcams",0.99,"webcam");
  if(any(t,DOCK)) return result("docking_station","Peripherie","Dockingstationen",0.99,"dock");
  if(any(t,NETCABLE)) return result("network_cable_adapter","Netzwerk","Kabel & Adapter",0.995,"network cable/adapter");
  if(any(t,ROUTER)) return result("router","Netzwerk","Netzwerk",0.99,"router");
  if(any(t,SWITCH)) return result("network_switch","Netzwerk","Netzwerk",0.99,"network switch");
  if(any(t,STORAGE)) return result("storage_device","Datenspeicher","Storage",0.985,"storage");
  if(any(t,PRINTER)&&!any(t,PRINTCON)) return result("printer_scanner","Office & Business","Drucker & Scanner",0.99,"printer/scanner");
  ...
  if(any(t,ROBOT)) return result("robot_vacuum","Smart Home","Gebäudetechnik",0.97,"robot vacuum - review target");
...
const allPath=path.join(OUT,`catalog-classifier-v1-all-${stamp}.csv`), safePath=path.join(OUT,`catalog-classifier-v1-safe-changes-${stamp}.csv`), reviewPath=path.join(OUT,`catalog-classifier-v1-review-${stamp}.csv`), riskPath=path.join(OUT,`catalog-classifier-v1-page-risk-${stamp}.csv`), summaryPath=path.join(OUT,`catalog-classifier-v1-summary-${stamp}.txt`);
writeCsv(allPath,rows);writeCsv(safePath,safe);writeCsv(reviewPath,review);writeCsv(riskPath,[...risk,...unknown]);

const lines=[];
lines.push("========== IUMATEC CATALOG CLASSIFIER V1 ==========","MODO: DRY RUN — NENHUM produto foi alterado.","",`SKUs únicos analisados: ${rows.length}`,`Classificados e já corretos: ${clean.length}`,`Mudanças seguras propostas: ${safe.length}`,`Revisão manual: ${review.length}`,`Sem regra forte / manter: ${keep.length}`,`Contaminações certas em páginas principais: ${risk.length}`,`Casos não classificados em páginas principais: ${unknown.length}`,"","=== TOP PRODUCT TYPES ===");
...
lines.push("","Relatórios:",allPath,safePath,reviewPath,riskPath,"","IMPORTANTE: este V1 NÃO tem APPLY.","Primeiro reveja o relatório. Depois criamos o APPLY separado e protegido.");
