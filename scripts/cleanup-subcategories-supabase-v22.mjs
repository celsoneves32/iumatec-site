...
if (APPLY && CONFIRM !== "IUMATEC-SANITY-V22") {
  throw new Error(
    "APPLY bloqueado. Use: --apply --confirm=IUMATEC-SANITY-V22"
  );
}
...
async function loadAll() {
  // IMPORTANT:
  // The old V2/V2.1 used OFFSET/RANGE pagination without ORDER BY.
  // PostgreSQL does not guarantee row order without ORDER BY, so across
  // 52k+ rows the same SKU could reappear while another SKU was skipped.
  //
  // V2.2 uses deterministic KEYSET pagination by SKU instead.
  // We intentionally keep ONE representative row per SKU because APPLY
  // also updates by SKU (.eq("sku", ...)).
  const out = [];
  const seen = new Set();
  const pageSize = 1000;
  let cursor = null;
  ...
  while (true) {
    ...
    for (const row of data) {
      const sku = txt(row.sku);
      if (!sku || seen.has(sku)) continue;
      seen.add(sku);
      out.push(row);
    }

    const lastSku = txt(data[data.length - 1]?.sku);

    if (!lastSku) {
      throw new Error("Paginacao V2.2: ultimo SKU vazio; abortado por seguranca.");
    }

    if (cursor !== null && lastSku <= cursor) {
      throw new Error(
        `Paginacao V2.2 nao avancou (cursor=${cursor}, lastSku=${lastSku}).`
      );
    }

    cursor = lastSku;
    console.log(`SKUs unicos lidos: ${out.length} | cursor: ${cursor}`);
...
const report = path.join(
  OUT,
  `subcategory-sanity-v22-${APPLY ? "apply" : "dry-run"}-${stamp}.csv`
);
...
console.log("");
console.log("========== IUMATEC SUBCATEGORY SANITY V2.2 ==========");
console.log("Paginacao: KEYSET por SKU (deterministica)");
console.log(`SKUs unicos analisados: ${products.length}`);
...
if (!APPLY) {
  ...
  console.log(
    "node .\\scripts\\cleanup-subcategories-supabase-v22.mjs " +
    "--apply --confirm=IUMATEC-SANITY-V22"
  );
} else {
  console.log("");
