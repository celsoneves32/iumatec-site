...
/*
 IUMATEC FINAL CATEGORY GUARD V1.5.2 — DRY RUN ONLY
 =================================================
 Foco:
   - Computer > Laptops
   - Peripherie > Monitore
...
const LAPTOP_KEYBOARD_PERIPHERAL = [
  /\btastatur\b/,
  /\bkeyboard\b/,
  /\bmk470\b/,
  /\bdesktop\s+(?:set|combo)\b/,
];
...
const DESKTOP_PC = [
  // Only strong desktop/AIO signals. Do NOT use the word "desktop" by itself:
  // keyboard/mouse bundles often contain "Desktop" in their product title.
  /\bdesktop\s+(?:pc|computer)\b/,
  /\b(?:pc|computer)\s+desktop\b/,
  /\b(?:all[\s-]?in[\s-]?one|aio)\b/,
  /\bprodesk\b/,
  /\belitedesk\b/,
  /\boptiplex\b/,
  /\b(?:z1|z2)\s+(?:tower|sff)\b/,
  /\bmedion\s+multiworx\b/,
  /\bmedion\s+signium\b.*\b(?:aio|27(?:\.\d+)?|23\.8)\b/,
...
function classifyLaptop(p) {
  ...
  if (hit(t, LAPTOP_KEYBOARD_PERIPHERAL)) {
    return result(
      "SAFE",
      "Peripherie",
      "Tastaturen",
      "keyboard_or_desktop_set",
      "laptop-page-keyboard-peripheral"
    );
  }
...
function runTests() {
  const tests = [
    ...
    ["Laptop false positive Medion Signium 14", classifyLaptop, {brand:"Medion",title:"Medion Signium 14 S1, Intel Core 5 120U 14 WQXGA+, 16GB, 512GB SSD, W11-H",category:"Computer",subcategory:"Laptops"}, "Laptops"],
    ["Laptop false positive Captiva 16", classifyLaptop, {brand:"Captiva",title:"Captiva Highend Gaming R98-336CH RTX5080 16 WQXGA, AMD R9, 32GB, 2 TB SSD",category:"Computer",subcategory:"Laptops"}, "Laptops"],
    ["Desktop word keyboard", classifyLaptop, {brand:"Logitech",title:"Logitech MK470 Desktop USB 2.4 GHz",category:"Computer",subcategory:"Laptops"}, "Tastaturen"],
    ["Desktop word lock", classifyLaptop, {brand:"Kensington",title:"Kensington Schloss Desktop/ Peripherie 2.0 Schlosstyp Normal",category:"Computer",subcategory:"Laptops"}, "Computer-Zubehör"],
...
const safePath = path.join(OUT, `final-category-guard-v152-safe-${stamp}.csv`);
const reviewPath = path.join(OUT, `final-category-guard-v152-review-${stamp}.csv`);

writeCsv(safePath, safe);
writeCsv(reviewPath, review);

console.log("");
console.log("========== IUMATEC FINAL CATEGORY GUARD V1.5.2 ==========");
