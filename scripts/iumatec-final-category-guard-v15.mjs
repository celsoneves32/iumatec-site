...
/*
 IUMATEC FINAL CATEGORY GUARD V1.5 — DRY RUN ONLY
 =================================================
 Foco:
   - Computer > Laptops
   - Peripherie > Monitore
...
const PHONE_ACCESSORY = [
  /\b(?:iphone|smartphone|galaxy|pixel)\b.*\b(?:case|cover|hulle|huelle|folio|bumper)\b/,
  /\b(?:case|cover|hulle|huelle|folio|bumper)\b.*\b(?:iphone|smartphone|galaxy|pixel)\b/,
...
const safePath = path.join(OUT, `final-category-guard-v15-safe-${stamp}.csv`);
const reviewPath = path.join(OUT, `final-category-guard-v15-review-${stamp}.csv`);

writeCsv(safePath, safe);
writeCsv(reviewPath, review);

console.log("");
console.log("========== IUMATEC FINAL CATEGORY GUARD V1.5 ==========");
