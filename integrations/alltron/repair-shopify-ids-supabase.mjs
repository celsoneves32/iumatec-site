...
async function shopifyGraphql(query, variables, attempt = 1) {
  const response = await fetch(`https://${SHOPIFY_DOMAIN}/admin/api/${SHOPIFY_VERSION}/graphql.json`, {
    method: "POST",
...
async function main() {
  ...
  if (result.ambiguous.length) fail("Existem correspondências ambíguas. A aplicação foi bloqueada por segurança.");
  const batches = [];
